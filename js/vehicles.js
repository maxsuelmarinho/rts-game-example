  ...
  defaults: {
    ...
    moveTo: function(destination, distanceFromDestination) {
      ...
      let vehicleOutsideMapBounds =
        start[1] < 0 ||
        start[1] > game.currentMap.mapGridHeight - 1 ||
        start[0] < 0 ||
        start[0] > game.currentMap.mapGridWidth;
    ...
    checkForCollisions: function(grid) {
      ...
      let gridHardCollisionThreshold = Math.pow(
        (this.radius * 0.9) / game.gridSize,
        2
      );
      let gridSoftCollisionThreshold = Math.pow(
        (this.radius * 1.1) / game.gridSize,
        2
      );
